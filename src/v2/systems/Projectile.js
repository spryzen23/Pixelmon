...
export default function Projectile({
  ...
  const checkWildCollision = () => {
    ...
    wildRefs.current.forEach((wild, wildId) => {
      ...
      const dist = projectile.position.distanceTo(wild.position);
      if (dist <= HIT_RADIUS) {
        console.log(`[Projectile] Hit ${wildId} at distance ${dist.toFixed(2)}`);
        startCapture(wildId, wild.position.clone());
      }
    });
  };

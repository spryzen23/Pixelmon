...
export function GameView() {
  ...
  useEffect(() => {
    ...
    setGameRuntime({
      spawnState: initBiomeSpawnState(
        info.regionId,
        {},
        spawnLadder || { regions: {} }
      ),
    ...
    const handleKeyDown = (e) => {
      ...
      if (player?.companions && player.companions.length > 0) {
        ...
        if (e.code === "BracketLeft") {
          const currentIdx = player.companions.findIndex(
            (c) => c.entryId === player.companion?.entryId
          );
          const nextIdx =
            (currentIdx - 1 + player.companions.length) %
    ...
    const handleWheel = (e) => {
      ...
      setThrowPower((c) =>
        Math.max(
          ...
          Math.min(
            MAX_THROW_POWER,
            c + (e.deltaY < 0 ? THROW_POWER_STEP : -THROW_POWER_STEP)
          )
        )
      );
    };
    ...
    async (entry, isAlpha) => {
      ...
      try {
        ...
        if (equippedBallId !== "standard") {
          const remaining =
            result.player?.inventory?.balls?.[equippedBallId] ?? 0;
          if (remaining <= 0) {
        ...
        setGameRuntime((rt) => ({
          ...
          caughtCount:
            result.player?.perPathProgress?.[session.regionId]?.caughtEntryIds
              ?.length ?? rt.caughtCount + 1,
  ...
  const handleSandboxBiomeSwitch = useCallback(
    (pathId) => {
  ...
  if (isSandbox && (!gameRuntime || !session))
    return <LoadingOverlay currentPathId={sandboxPathId} isLoading />;
      ...
      >
        <div ref={uiContainerRef} id="ui-container">
          <div className="hud">
            ...
            <span>
              Click game window · WASD move · F/Space throw · E companion · X
              exit zones
            </span>
            ...
            <span>
              {formatSpawnProgressLine({
                ...
                eggGroups:
                  spawnProgress.eggGroups || spawnState?.activeEggGroups,
                ...
                return (
                  ...
                  >
                    ...
                    <div className="companion-hud-details">
                      <span className="companion-hud-name">
                        {comp.displayName}
                      </span>
                      <span className="companion-hud-types">
                        {comp.types?.join(" / ")}
                      </span>
                    </div>
          ...
          {isSandbox && (
            <div className="path-menu">
              <div className="path-pager-header">
                ...
                <span>
                  Biomes {pathPage + 1}/
                  {Math.ceil(WORLD_PATHS.length / PATHS_PER_PAGE)}
                </span>
                <button
                  type="button"
                  disabled={
                    pathPage >=
                    Math.ceil(WORLD_PATHS.length / PATHS_PER_PAGE) - 1
                  }
                  onClick={() => setPathPage((p) => p + 1)}
                >
                  ›
              ...
              <div className="path-list">
                {WORLD_PATHS.slice(
                  pathPage * PATHS_PER_PAGE,
                  (pathPage + 1) * PATHS_PER_PAGE
                ).map((biome) => (
                  <button
                    key={biome.id}
              ...
              <div className="pause-menu">
                ...
                <div className="style-picker">
                  ...
                  <div className="style-pager-header">
                    ...
                    <span>
                      {stylePage + 1}/
                      {Math.ceil(PLAYER_STYLES.length / STYLES_PER_PAGE)}
                    </span>
                    <button
                      type="button"
                      disabled={
                        stylePage >=
                        Math.ceil(PLAYER_STYLES.length / STYLES_PER_PAGE) - 1
                      }
                      onClick={() => setStylePage((p) => p + 1)}
                    >
                      ›

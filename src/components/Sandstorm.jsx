...
export default function Sandstorm({ playerRef }) {
  ...
  useEffect(() => {
    if (scene.fog?.isFogExp2) {
      scene.fog.color.set(DESERT_FOG_COLOR);
      scene.fog.density = DESERT_FOG_DENSITY;
    }
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (scene.fog?.isFogExp2) {
      lerpFogExp2(
        scene,
        isStorming ? DESERT_STORM_FOG_COLOR : DESERT_FOG_COLOR,
        isStorming ? DESERT_STORM_FOG_DENSITY : DESERT_FOG_DENSITY
      );
    }
  });

...
function createWildCreatures(creatureModels = PILOT_POKE_MODELS) {
  const pool =
    creatureModels.length > 0 ? creatureModels : PILOT_POKE_MODELS;
  ...
  return Array.from({ length: count }, (_, index) => ({
    ...
    status: 'active',
    modelUrl: pool[Math.floor(Math.random() * pool.length)],
  }));
}
...
export default function GameScene({
  ...
  const [wildCreatures, setWildCreatures] = useState(() =>
    createWildCreatures(PILOT_POKE_MODELS)
  );

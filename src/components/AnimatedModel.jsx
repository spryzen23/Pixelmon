...
export default function AnimatedModel({
  ...
  const isFloatingLoco =
    profile?.locomotion === "float" || profile?.locomotion === "aerial";
  const procWeights = profile?.procedural || {};
  const swingTime = useRef(0);
  const resolvedClipName = resolveAnimationClip(
    names,
    actionName,
    fallbackActionName
  );
  ...
  useFrame((state, delta) => {
    ...
    const knee =
      Math.sin(swingTime.current + 0.35) * (isSprinting ? 0.45 : 0.28);
    ...
    if (group) {
      const bob =
        isWalking && !liveJumping && !liveCrouching
          ? Math.abs(Math.sin(swingTime.current)) *
            (isSprinting ? 0.055 : 0.035)
          : isFloatingLoco
      ...
      const forwardLean = isWalking
        ? Math.max(-1, Math.min(1, liveForward)) *
          (isSprinting ? -0.095 : -0.045)
        : 0;
    ...
    scene.traverse((node) => {
      ...
      if (!skipFullProcedural) {
        ...
        if (matchesCategory(name, "head") || lower.includes("head")) {
          node.rotation.y =
            origRot.y + Math.max(-0.85, Math.min(0.85, liveLookAngle));
          node.rotation.x =
            origRot.x - Math.max(-0.45, Math.min(0.45, liveLookPitch));
        }
        return;
      }
      ...
      if (isSerpent) {
        ...
        if (isSpineJoint || isTailJoint) {
          ...
          if (isWalking) {
            ...
            const coilVal =
              Math.sin(idleTime.current * 1.1 + jointIdx * 0.35) * 0.05;
      ...
      if (liveCrouching) {
        ...
        const crouchKnee = isWalking
          ? Math.sin(swingTime.current + 0.35) * 0.12
          : 0;
        const legSwing = matchesCategory(name, "upperLeg")
          ? crouchStride * legSign
          : crouchKnee * legSign;

        if (
          matchesCategory(name, "upperLeg") ||
          matchesCategory(name, "lowerLeg")
        ) {
          node.rotation.x = origRot.x - 0.65 + legSwing;
          node.rotation.z = origRot.z + (left ? 0.14 : -0.14);
        } else if (matchesCategory(name, "foot")) {
          node.rotation.x =
            origRot.x + 0.42 - (isWalking ? crouchStride * legSign * 0.1 : 0);
        } else if (matchesCategory(name, "spine")) {
          node.rotation.x =
            origRot.x -
            0.28 +
            (isWalking ? Math.sin(swingTime.current * 2) * 0.02 : 0);
        } else if (matchesCategory(name, "head")) {
          node.rotation.x =
            origRot.x +
            0.16 +
            (isWalking ? Math.sin(swingTime.current * 2) * 0.03 : 0);
          node.rotation.y =
            origRot.y + Math.max(-0.85, Math.min(0.85, liveLookAngle));
          ...
          const armSwing = isWalking
            ? Math.sin(swingTime.current) * 0.15 * armSign
            : 0;
          ...
          const armSwing = isWalking
            ? Math.sin(swingTime.current) * 0.12 * armSign
            : 0;
        ...
        if (
          ...
          node.rotation.x =
            origRot.x +
            (matchesCategory(name, "upperLeg") ? hipFlex : kneeFlex);
        } else if (matchesCategory(name, "foot")) {
          ...
          node.rotation.x =
            origRot.x - 0.12 - riseFactor * 0.1 + fallFactor * 0.05;
        } else if (matchesCategory(name, "head")) {
        ...
        if (matchesCategory(name, "upperLeg")) {
          ...
          const armAngle = isSprinting
            ? stride * 1.15 * armSign - 0.3
            : stride * 0.65 * armSign;
          ...
          const lowerArmAngle = isSprinting
            ? stride * 0.75 * armSign
            : stride * 0.42 * armSign;
          node.rotation.x = origRot.x + lowerArmAngle;
        } else if (
          matchesCategory(name, "wing") &&
          (procWeights.wing || 0) > 0
        ) {
          const w = procWeights.wing || 0.5;
          node.rotation.x =
            origRot.x + Math.sin(swingTime.current * 1.4) * 0.35 * w;
          node.rotation.z =
            origRot.z +
            Math.cos(swingTime.current * 1.2) * 0.2 * w * (left ? 1 : -1);
          ...
          matchesCategory(name, "antenna") &&
          (procWeights.antenna || 0) > 0
        ) {
          const w = procWeights.antenna || 0.4;
          node.rotation.x =
            origRot.x + Math.sin(swingTime.current * 2) * 0.15 * w;
        } else if (matchesCategory(name, "tail")) {
          const tailW = procWeights.tail || 0.3;
          node.rotation.z =
            origRot.z + Math.sin(swingTime.current * 0.7) * 0.12 * tailW;
          node.rotation.y =
            origRot.y + Math.sin(swingTime.current * 0.5) * 0.06 * tailW;
        } else if (matchesCategory(name, "head")) {
          node.rotation.x = origRot.x + Math.sin(swingTime.current * 2) * 0.05;
          node.rotation.y =
            origRot.y + Math.max(-0.85, Math.min(0.85, liveLookAngle));
          node.rotation.x -= Math.max(-0.45, Math.min(0.45, liveLookPitch));
        } else if (matchesCategory(name, "spine")) {
          node.rotation.x = origRot.x + Math.sin(swingTime.current * 2) * 0.025;
        ...
        if (matchesCategory(name, "float") && (procWeights.float || 0) > 0) {
          ...
          node.rotation.y =
            origRot.y + Math.sin(idleTime.current * 0.8) * 0.08 * tailW;
        } else if (
          matchesCategory(name, "wing") &&
          (procWeights.wing || 0) > 0
        ) {
          ...
          matchesCategory(name, "vine") &&
          (procWeights.vine || 0) > 0
        ) {
          const w = procWeights.vine || 0.5;
          node.rotation.x =
            origRot.x + Math.sin(idleTime.current * 1.2) * 0.08 * w;
          node.rotation.z =
            origRot.z +
            Math.cos(idleTime.current * 1.0) * 0.04 * w * (left ? 1 : -1);
        }
      }
  ...
  useEffect(() => {
    try {
      const clipName = resolveAnimationClip(
        names,
        actionName,
        fallbackActionName
      );

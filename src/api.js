...
export const api = {
  ...
  getSpawns: (regionId, level) =>
    request(
      `/api/biomes/${regionId}/spawns${level != null ? `?level=${level}` : ""}`
    ),
  ...
  startSession: (id, body, options = {}) =>
    ...
    request(`/api/players/${id}/catch`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  mapComplete: (id, body) =>
    request(`/api/players/${id}/map-complete`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  startBattle: (body) =>
  ...
  submitBattleChoice: (body) =>
    request("/api/battle/choice", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  createBattleEngineSession: (body) =>
    request("/api/battle-engine/sessions", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  getBattleEngineSession: (battleId) =>
    request(`/api/battle-engine/sessions/${battleId}`),
  submitBattleEngineChoice: (battleId, body) =>
    request(`/api/battle-engine/sessions/${battleId}/choices`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  getBattleEngineMove: (id) =>
    request(`/api/battle-engine/catalog/moves/${id}`),
  getBattleEngineAbility: (id) =>
    request(`/api/battle-engine/catalog/abilities/${id}`),
  getBattleEngineItem: (id) =>
    request(`/api/battle-engine/catalog/items/${id}`),
  login: (body) =>

...
const Abilities = {
    ...
    onResidual(pokemon) {
      for (const allyActive of pokemon.adjacentAllies()) {
        if (allyActive.status && this.randomChance(3, 10)) {
          ...
          allyActive.cureStatus(false, pokemon);
        }
      }
    }
  },
  ...
  naturalcure: {
    ...
    onSwitchOut(pokemon) {
      ...
      pokemon.cureStatus(pokemon.showCure, pokemon);
  ...
  embodyaspectpixiedust: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.name === "Ogerpon-Pixiedust-Tera" && pokemon.terastallized && !this.effectState.embodied) {
  ...
  stancechange: {
    ...
    onModifyMove(move, attacker, defender) {
      ...
      if (move.category !== "Status" && move.id === "soulboundslash" && attacker.species.forme !== "Aegislash-Soulbound") attacker.formeChange("Aegislash-Soulbound");
      if (move.category !== "Status" && move.id !== "soulboundslash" && attacker.species.forme === "Aegislash") attacker.formeChange("Aegislash-Blade");
    },
  ...
  bristle: {
    onDamage(damage, target, source, effect) {
      if (effect.effectType === "Move" && !effect.multihit && !(effect.hasSheerForce && source.hasAbility("sheerforce") && effect.category === "Physical")) {
  ...
  cashinfusion: {
    ...
    name: "Cash Infusion",
    rating: 1.5,
    shortDesc: "This Pok\xE9mon receives a damage boost equal to 2% of the Pok\xE9mon Dollars scattered throughout a battle."
  },
  crystalize: {
    onAfterMoveSecondarySelf(source, target, move) {
    ...
    name: "Crystalize",
    rating: 3,
    shortDesc: "If this Pok\xE9mon uses a Rock-type move before the foe attacks, return 1.5x damage received to attackers for the rest of this turn."
  },
  bellchoir: {
    onAfterMoveSecondarySelf(source, target, move) {
      if (move.flags["sound"]) {

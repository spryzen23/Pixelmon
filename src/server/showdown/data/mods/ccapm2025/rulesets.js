  ...
  ccapmformchanges: {
    ...
    onSwitchIn(pokemon) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        ...
        if (pokemon.species.name === "Jirachi" && pokemon.side.holdHandsUsers?.length >= 2) {
          pokemon.formeChange("Jirachi-Harmonic", null, true);
          const holdHandsIndex = pokemon.set.moves.map((move2) => move2.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("holdhands");
      ...
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        ...
        if (weather === "sunnyday" || weather === "desolateland") {
          if (target.species.name === "Beartic") {
            ...
            const glacierFangIndex = target.set.moves.map((move2) => move2.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("glacierfang");
          ...
          if (target.species.name === "Beartic-Freshwater") {
            ...
            const meltingMaulIndex = target.set.moves.map((move2) => move2.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("meltingmaul");
    ...
    onDamagingHit(damage, target, source, move) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        if (move.type === "Fire" && target.species.name === "Beartic") {
          ...
          const glacierFangIndex = target.set.moves.map((setMove) => setMove.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("glacierfang");
          if (glacierFangIndex < 0) return;
          ...
          const meltingMaulIndex = target.set.moves.map((setMove) => setMove.toLowerCase().replace(/[^a-z0-9]/g, "")).indexOf("meltingmaul");
          if (meltingMaulIndex < 0) return;
        ...
        } else if (!move.damage && !move.damageCallback && target.getMoveHitData(move).typeMod > 0 && target.species.name === "Rhyperior") {
    ...
    onAfterBoost(boost, target, source, effect) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        if (boost.spe && boost.spe > 0 && target.species.name === "Blaziken") {
          target.formeChange("Blaziken-Wildfire", null, true);
        }
      }
      if (effect?.name === "Fiery Dance" && boost.spa && source.species.name === "Volcarona") {
    ...
    onAfterMoveSecondarySelf(source, target, move) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        ...
        } else if (source.species.name === "Luxray-Conductive" && move.type !== "Electric") {
    ...
    onResidual(pokemon) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        ...
        } else if (pokemon.hp <= pokemon.maxhp / 2 && pokemon.species.name === "Sunflora") {
          pokemon.formeChange("Sunflora-Wilted", null, true);
        } else if (pokemon.hp <= pokemon.maxhp / 2 && pokemon.species.name === "Torterra") {
    ...
    onUpdate(pokemon) {
      if (!this.ruleTable.tagRules.includes("+pokemontag:cap")) {
        ...
        for (const target of pokemon.adjacentFoes()) {
          if ((target.status === "psn" || target.status === "tox") && pokemon.species.name === "Pecharunt") {
            pokemon.formeChange("Pecharunt-Puppetmaster", null, true);
          } else if (target.status !== "psn" && target.status !== "tox" && pokemon.species.name === "Pecharunt-Puppetmaster") {
            pokemon.formeChange("Pecharunt", null, true);
          }
        }
        if (pokemon.species.name === "Victini" && pokemon.hp <= pokemon.maxhp / 4 && pokemon.hp > 0 && pokemon.side.pokemonLeft > 1) {
          let stat;

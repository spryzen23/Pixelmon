...
const ENEMY_SETS = {
  rattata: {
    species: "Rattata",
    level: 50,
    moves: ["Tackle", "Quick Attack", "Bite", "Tail Whip"],
  },
  pidgey: {
    species: "Pidgey",
    level: 50,
  ...
  zubat: {
    species: "Zubat",
    level: 50,
    moves: ["Bite", "Wing Attack", "Confuse Ray", "Supersonic"],
  },
  geodude: {
    species: "Geodude",
    level: 50,
    moves: ["Tackle", "Rock Throw", "Mud Slap", "Defense Curl"],
  },
  ekans: {
    species: "Ekans",
    level: 50,
    moves: ["Bite", "Poison Sting", "Wrap", "Glare"],
  },
  sandshrew: {
    species: "Sandshrew",
    level: 50,
    moves: ["Scratch", "Sand Tomb", "Poison Sting", "Defense Curl"],
  },
  charizard: {
    species: "Charizard",
    level: 50,
  ...
  gengar: {
    species: "Gengar",
    level: 50,
    moves: ["Shadow Ball", "Sludge Bomb", "Dazzling Gleam", "Hypnosis"],
  },
  garchomp: {
    species: "Garchomp",
    level: 50,
    moves: ["Earthquake", "Dragon Claw", "Rock Slide", "Swords Dance"],
  },
  dragonite: {
    species: "Dragonite",
    level: 50,
    moves: ["Outrage", "Hurricane", "Fire Punch", "Roost"],
  },
  metagross: {
    species: "Metagross",
    level: 50,
  ...
  gyarados: {
    species: "Gyarados",
    level: 50,
    moves: ["Waterfall", "Bounce", "Crunch", "Dragon Dance"],
  },
  mewtwo: {
    species: "Mewtwo",
    level: 70,
    moves: ["Psystrike", "Shadow Ball", "Aura Sphere", "Recover"],
  },
  rayquaza: {
    species: "Rayquaza",
    level: 70,
    moves: ["Dragon Ascent", "Outrage", "Extreme Speed", "Dragon Dance"],
  },
  arceus: {
    species: "Arceus",
    level: 70,
    moves: ["Judgment", "Recover", "Extreme Speed", "Earthquake"],
  },
  kyogre: {
    species: "Kyogre",
    level: 70,
    moves: ["Origin Pulse", "Ice Beam", "Thunder", "Calm Mind"],
  },
  groudon: {
    species: "Groudon",
    level: 70,
  ...
  giratina: {
    species: "Giratina",
    level: 70,
    moves: ["Shadow Force", "Draco Meteor", "Will-O-Wisp", "Hex"],
  },
};
...
function normalizeClientSpecies(value) {
  ...
  if (clientName.startsWith("pikachu-") && clientName.endsWith("-cap")) {
    return (
      getKnownSpeciesName(clientName.replace("-cap", "")) ||
      capitalizeWords(clientName.replace("-cap", ""))
    );
  }

  const direct =
    getKnownSpeciesName(clientName) ||
    getKnownSpeciesName(capitalizeWords(clientName));
  ...
  const normalizedDirect =
    getKnownSpeciesName(normalized) ||
    getKnownSpeciesName(capitalizeWords(normalized));
  ...
  for (let length = parts.length - 1; length >= 1; length -= 1) {
    ...
    const species =
      getKnownSpeciesName(trimmed) ||
      getKnownSpeciesName(capitalizeWords(trimmed));
...
function mapClientPokemonToBattleReady(pokemon, index) {
  const species = normalizeClientSpecies(
    pokemon.name || pokemon.species || pokemon.displayName
  );
  ...
  return {
    id:
      pokemon.id ||
      `p1-${index + 1}-${String(pokemon.name || species).toLowerCase()}`,
    species,
...
function buildEnemyPokemon(setKey, index = 0) {
  ...
  const levelScale =
    template.level >= 70 ? 2.2 : template.level >= 50 ? 1.6 : 1;
...
function getEnemyTeam(difficulty, formatId, battleFormat) {
  ...
  if (battleFormat === "singles6v6") {
    ...
    formatId === "gen7doublescustomgame" ||
    battleFormat === "doubles"
  ) {
    teamSize = 6;
  } else if (
    formatId === "gen6triplescustomgame" ||
...
function getTrainerParticipant(formatId, battleFormat) {
  const trainer =
    TRAINER_PARTICIPANTS[
      Math.floor(Math.random() * TRAINER_PARTICIPANTS.length)
    ];
  let teamSize = 3;
  if (battleFormat === "singles6v6") teamSize = 6;
  if (formatId === "gen7doublescustomgame" || battleFormat === "doubles")
    teamSize = 6;
  if (formatId === "gen6triplescustomgame" || battleFormat === "triples")
    teamSize = 6;

  let team = trainer.team.map((pokemon) => ({
    ...pokemon,
    stats: { ...pokemon.stats },
  }));
...
function getEnemyParticipant(difficulty, formatId, battleFormat) {
  if (difficulty === "trainer3v3")
    return getTrainerParticipant(formatId, battleFormat);
  return {
    side: "p2",

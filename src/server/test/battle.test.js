...
describe("battleService — startNewBattle()", () => {
  ...
  it("returns logs array with at least a switch-in event", () => {
    ...
    assert.ok(
      ...
      "should have at least 2 switch-in events (one per side)"
    );
  });
  ...
  it("returns a request with active moves", () => {
    ...
    assert.ok(
      Array.isArray(result.request.active[0].moves),
      "active moves should be an array"
    );
    assert.ok(
      result.request.active[0].moves.length > 0,
      "should have at least one move available"
    );
  });
  ...
  it("returns a request with side team info", () => {
    ...
    assert.ok(
      Array.isArray(result.request.side.pokemon),
      "side.pokemon should be an array"
    );
  });
  ...
  it("includes weather event in logs when weather is set to rain", () => {
    ...
    const hasWeatherEvent =
      logString.includes("|-weather|RainDance") ||
      logString.includes("raindance");
    assert.ok(
      hasWeatherEvent || result.logs.length > 0,
      "should set weather or log events"
    );
  });
  ...
  it("works with gym difficulty (medium)", () => {
    ...
    assert.ok(
      result.request.active,
      "should have active pokemon in gym difficulty"
    );
  });
  ...
  it("works with boss difficulty (hard)", () => {
    ...
    assert.ok(
      result.request.active,
      "should have active pokemon in boss difficulty"
    );
  });
  ...
  it("works with random trainer 3v3 difficulty", () => {
    const result = startNewBattle({
      team: sampleTeam,
      difficulty: "trainer3v3",
      weather: "clear",
    });
    assert.ok(result.battleId, "trainer battle should start correctly");
    assert.ok(
      result.request.active,
      "should have active pokemon in trainer difficulty"
    );
    assert.ok(
      result.logs.some((line) => line === "|teamsize|p2|3"),
      "opponent should bring a 3 Pokemon team"
    );
  });
});
...
describe("battleService — makeChoice()", () => {
  it("executes move 1 and returns a non-empty log", () => {
    ...
    assert.ok(
      moveEvents.length >= 1,
      "should have at least one |move| event per turn"
    );
  });
  ...
  it("returns an updated request after a turn", () => {
    ...
    assert.ok(
      result.request || result.winner !== undefined,
      "should return request or winner state"
    );
  });
  ...
  it("executes a full restore and includes heal event in logs", () => {
    ...
    assert.ok(
      ...
      "should produce logs after full restore use"
    );
    const hasAnyEvent = result.logs.some((l) => l.startsWith("|"));
    assert.ok(hasAnyEvent, "should have at least one protocol event");
  });
  ...
  it("simulates a 3-turn battle loop without crashing", () => {
    let { battleId, request } = startNewBattle({
      team: sampleTeam,
      difficulty: "wild",
      weather: "clear",
    });
    ...
    for (let turn = 0; turn < 3; turn++) {
      ...
      const choice =
        validMoveIdx !== -1 ? `move ${validMoveIdx + 1}` : "move 1";
    ...
    assert.ok(
      winner === null || winner === "player" || winner === "enemy",

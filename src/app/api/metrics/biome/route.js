...
export async function POST(request) {
  try {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        console.warn('Could not read existing metrics file, starting fresh', err);
      }
    }
    ...
    await fs.writeFile(METRICS_FILE_PATH, JSON.stringify(existingData, null, 2), 'utf8');

    return NextResponse.json({ success: true, count: existingData.events.length });
  } catch (error) {

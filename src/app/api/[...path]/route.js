...
export async function GET(request, { params }) {
  try {
    ...
    if (
      routeParts[0] === "battle-engine" &&
      routeParts[1] === "catalog" &&
      routeParts.length === 4
    ) {
      const [, , kind, id] = routeParts;
      const record =
        kind === "moves"
          ? battleEngineCatalog.getMove(id)
          : kind === "abilities"
            ? battleEngineCatalog.getAbility(id)
            : kind === "items"
              ? battleEngineCatalog.getItem(id)
              : null;
      if (!record)
        return NextResponse.json(
...
export async function POST(request, { params }) {
  try {
    ...
    if (routeParts[0] === "auth" && routeParts[1] === "register") {
      ...
      return NextResponse.json(
        { success: true, token: user.id, user },
        { status: 201 }
      );
    }
    ...
    if (routeParts[0] === "auth" && routeParts[1] === "login") {
      ...
      const db = await getDB();
      const row = await db.get("SELECT data FROM players WHERE username = ?", [
        username.toLowerCase().trim(),
      ]);
      if (!row) {
        return NextResponse.json(
    ...
    if (
      routeParts[0] === "players" &&
      routeParts[2] === "session" &&
      routeParts[3] === "start"
    ) {
      const id = routeParts[1];
    ...
    if (routeParts[0] === "battle" && routeParts[1] === "choice") {
      ...
      if (!battleId || !choice) {
        return NextResponse.json(
          { error: "battleId and choice are required" },
          { status: 400 }
        );
      }
    ...
    if (
      ...
      return NextResponse.json(createBattleEngineSession(body), {
        status: 201,
      });
    }
    ...
    if (
      ...
      return NextResponse.json(
        submitBattleEngineChoice(routeParts[2], side, choice)
      );
    }

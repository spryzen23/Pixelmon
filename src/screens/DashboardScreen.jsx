...
export function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    if (!player && user.trainers && user.trainers.length > 0) {
      ...
      const activeTrainer =
        user.trainers.find((t) => t.id === lastId) || user.trainers[0];
      if (activeTrainer) {
        api
          .getPlayer(activeTrainer.id)
          .then((t) => setPlayer(t))
          .catch(console.error);
      }
    }
  ...
  useEffect(() => {
    if (biomeMap?.regions && biomeMap.regions.length > 0 && !selectedRegion) {
      ...
      const firstPlayable =
        biomeMap.regions.find((r) => r.playable) || biomeMap.regions[0];
  ...
  const handleBuyBall = async (ballId, cost) => {
    ...
    try {
      const { result, newCoins } = await purchaseBall(
        player,
        user,
        ballId,
        cost
  ...
  const completionPct =
    totalMaps > 0 ? Math.round((completedPaths.size / totalMaps) * 100) : 0;
            ...
            <div className="db-panel db-tab-panel">
              ...
              {!player ? (
                <div className="onboarding-trainer-panel">
                  ...
                  <p>
                    Welcome! Create a Trainer profile to start catching voxel
                ...
                <>
                  ...
                  <div className="db-preview-container">
                    <button
                      ...
                      onClick={() =>
                        setPreviewType((t) =>
                          t === "companion" ? "trainer" : "companion"
                        )
                      }
                    >
                      View{" "}
                      {previewType === "companion" ? "Trainer" : "Companion"}
                    </button>
                    ...
                    {previewType === "companion" && player.companion ? (
                      ...
                      <CompanionPreview
                        modelUrl={
                          player.characterStyle?.modelUrl ||
                          "/assets/players/player%20(21).glb"
                        }
                        primaryType="normal"
                        fitToHeight={1.25}
                        isFloating={false}
                    ...
                    <div className="trainer-modal-list">
                      {trainers
                        ...
                        .map((t) => (
                          ...
                          >
                            {t.displayName} ({t.completedPathIds?.length ?? 0}{" "}
                            clears)
                          </button>
                        ))}
                    </div>
              ...
              <div className="db-companions-grid">
                {player.companions.slice(0, 5).map((comp, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="slot-initial">
                        {comp.displayName.slice(0, 2).toUpperCase()}
                      </div>
              ...
              <div className="shop-grid">
                ...
                <div className="shop-card">
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(circle at 32% 28%, #ffffff 0 30%, #e61f2c 30% 100%)",
                    }}
                  />
                  <div className="shop-card-name">Standard</div>
                  <div className="shop-card-qty">Qty: ∞</div>
                  <button className="shop-buy-btn" disabled>
                    FREE
                  </button>
                </div>
                ...
                {shopItems.map((item) => {
                  const owned =
                    player.inventory?.balls?.[item.id] ??
                    (item.id === "great" ? 5 : 1);
                  ...
                  return (
                    <div className="shop-card" key={item.id}>
                      ...
                      <div
                        className="shop-card-preview"
                        style={{
                          background: `radial-gradient(circle at 32% 28%, ${item.accentColor} 0 30%, ${item.color} 30% 100%)`,
                        }}
                      />
          ...
          <div className="modes-grid">
            ...
            <div className="mode-hub-card mode-campaign">
              ...
              <div className="mode-card-content">
                <p className="mode-card-desc">
                  Progress biome-by-biome to complete your Dex. Catch all spawns
                  in a region to unlock and advance to the next element region!
                </p>
                ...
                {player && biomeMap?.regions && (
                  <div className="inline-map-selector">
                    <span className="inline-map-selector-title">
                      Select Region Map
                    </span>
                    ...
                    <div className="db-map-grid-inline">
                      {biomeMap.regions.map((region) => {
                        const isUnlocked =
                          region.playable && unlockedPaths.has(region.pathId);
                        ...
                        return (
                          ...
                          >
                            <span className="db-map-name-inline">
                              {display.terrainName}
                            </span>
                            <span className="db-map-meta-inline">
                              {isDone
                                ? "🏆 Cleared"
                                : `Lvl ${region.minDex}–${region.maxDex}`}
                            </span>
                          </button>
                        );
                      })}
                ...
                <button
                  ...
                  disabled={
                    !player ||
                    !selectedRegion ||
                    !unlockedPaths.has(selectedRegion?.pathId)
                  }
            ...
            {showV2Features && (
              <div
                ...
                style={{ borderTop: "2px solid #7cb342" }}
              >
                <div className="mode-card-header">
                  <span className="mode-card-icon">🗺️</span>
                  <div className="mode-card-title-group">
                ...
                <div className="mode-card-content">
                  <p className="mode-card-desc">
                    Experience the newly optimized, chunk-based biome map
                    loading logic.
                  </p>
                  <button
                    className="launch-mode-btn"
            ...
            {showV2Features && (
              ...
              >
                ...
                <div className="mode-card-content">
                  <p className="mode-card-desc">
                    Explore a smooth, hand-crafted open world testing region
            ...
            {showV2Features && (
              <div
                className="mode-hub-card mode-campaign"
                style={{ borderTop: "2px solid #ff9800" }}
              >
                <div className="mode-card-header">
                  <span className="mode-card-icon">🛠️</span>
                  <div className="mode-card-title-group">
                ...
                <div className="mode-card-content">
                  <p className="mode-card-desc">
                    Tweak procedural terrain settings and export static .GLB
                    chunks for the Hisui Prototype.
                  </p>
                  <button
                    className="launch-mode-btn"
            ...
            <div className="mode-hub-card mode-sandbox">
              ...
              <div className="mode-card-content">
                <p className="mode-card-desc">
                  Explore all voxel biomes freely with advanced spawns,
                  real-time biome metrics export, and developer spawn tracking.
                  No level limits or locks!
                </p>
                <button
                  className="launch-mode-btn"
            ...
            <div className="mode-hub-card mode-royale">
              ...
              <div className="mode-card-content">
                <p className="mode-card-desc">
                  Join dynamic online lobbies and catch as many voxel creatures
                  as possible within a tight timer. Compete live with other
                  trainers!
                </p>
                <button
                  className="launch-mode-btn"
            ...
            {showV2Features && (
              <div
                className="mode-hub-card mode-royale"
                style={{ borderTop: "2px solid #7cb342" }}
              >
                <div className="mode-card-header">
                  <span className="mode-card-icon">⚔️</span>
                ...
                <div className="mode-card-content">
                  <p className="mode-card-desc">
                    Join dynamic online lobbies and catch as many voxel
                    creatures as possible within a tight timer. Built on the V2
                    engine!
                  </p>
                  <button
                    className="launch-mode-btn"
                    onClick={handleStartRoyaleV2}
                  >
            ...
            <div className="mode-hub-card mode-minigames">
              ...
              <div className="mode-card-content">
                <p className="mode-card-desc">
                  Take on the 3v3 Battle Arena, answer daily PokéGrids, test
                  your knowledge in trivia, or play clue guesser. Earn massive
                  PokéCoins payouts!
                </p>
                <button
                  className="launch-mode-btn"
                  onClick={handleStartMinigames}
                >
                  Enter Arcade Hub
                </button>
              </div>

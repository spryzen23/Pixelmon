...
function generateTerrainGeometry(size, resolution, frequency, amplitude, flattenRadius) {
...
function generateDecorations(size, frequency, amplitude, flattenRadius, treeDensity, houseCount) {
  const trees = [];
  const houses = [];
...
export function MapEditorScreen() {
  ...
  const terrainGeo = useMemo(() => {
    return generateTerrainGeometry(size, resolution, frequency, amplitude, flattenRadius);
  }, [size, resolution, frequency, amplitude, flattenRadius]);

  const decorations = useMemo(() => {
    return generateDecorations(size, frequency, amplitude, flattenRadius, treeDensity, houseCount);
    ...
    exporter.parse(
      ...
      async (gltf) => {
        try {
          ...
          alert('Map saved successfully! You can now load it from the Region Select Lobby.');
        } catch (error) {
          alert(error.message);
        } finally {
          setIsSaving(false);
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', background: 'var(--px-sky-deep)', color: 'var(--px-text)', fontFamily: 'var(--px-font-sans)' }}>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <h2 style={{ margin: 0, fontFamily: 'var(--px-font-display)', fontSize: '1.5rem', color: 'var(--px-accent)' }}>Map Baker</h2>
        </div>

        {/* Terrain Generator Panel */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: 'var(--px-panel)', padding: '20px', borderRadius: 'var(--px-radius)', border: '1px solid var(--px-border)' }}>
          <h3 style={{ margin: '0', fontSize: 'var(--px-text-md)', color: 'white', borderBottom: '1px solid var(--px-border)', paddingBottom: '12px' }}>Terrain Settings</h3>
          
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Size (Units)</span>
              <span style={{ color: 'var(--px-accent)' }}>{size}</span>
            </div>
            <input type="range" min="50" max="1000" step="50" value={size} onChange={e => setSize(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <input type="range" min="16" max="256" step="16" value={resolution} onChange={e => setResolution(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Frequency (Bumps)</span>
              <span style={{ color: 'var(--px-accent)' }}>{frequency.toFixed(3)}</span>
            </div>
            <input type="range" min="0.01" max="0.2" step="0.01" value={frequency} onChange={e => setFrequency(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <input type="range" min="1" max="50" step="1" value={amplitude} onChange={e => setAmplitude(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <input type="range" min="0" max="200" step="10" value={flattenRadius} onChange={e => setFlattenRadius(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
        </div>

        {/* Decorations Panel */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: 'var(--px-panel)', padding: '20px', borderRadius: 'var(--px-radius)', border: '1px solid var(--px-border)' }}>
          <h3 style={{ margin: '0', fontSize: 'var(--px-text-md)', color: 'white', borderBottom: '1px solid var(--px-border)', paddingBottom: '12px' }}>Decorations & POIs</h3>
          
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Forest Density</span>
              <span style={{ color: 'var(--px-accent)' }}>{(treeDensity * 100).toFixed(0)}%</span>
            </div>
            <input type="range" min="0" max="0.5" step="0.01" value={treeDensity} onChange={e => setTreeDensity(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <input type="range" min="0" max="20" step="1" value={houseCount} onChange={e => setHouseCount(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>House Scale</span>
              <span style={{ color: 'var(--px-accent)' }}>{houseScale.toFixed(3)}</span>
            </div>
            <input type="range" min="0.01" max="0.1" step="0.005" value={houseScale} onChange={e => setHouseScale(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: 'var(--px-text-sm)', color: 'var(--px-text-muted)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>House Y-Offset</span>
              <span style={{ color: 'var(--px-accent)' }}>{houseYOffset.toFixed(1)}</span>
            </div>
            <input type="range" min="-5" max="5" step="0.1" value={houseYOffset} onChange={e => setHouseYOffset(Number(e.target.value))} style={{ accentColor: 'var(--px-accent)' }} />
          </label>
        </div>

        {/* Save/Export Panel */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: 'var(--px-panel)', padding: '20px', borderRadius: 'var(--px-radius)', border: '1px solid var(--px-border)' }}>
          <h3 style={{ margin: '0', fontSize: 'var(--px-text-md)', color: 'white', borderBottom: '1px solid var(--px-border)', paddingBottom: '12px' }}>Save & Export</h3>
        ...
        <Canvas camera={{ position: [0, 80, 120], fov: 60 }}>
          ...
          <group ref={groupRef}>
            ...
            {decorations.trees.map((tree, idx) => (
              <group key={`tree-${idx}`} position={tree.position} rotation={tree.rotation} scale={tree.scale}>
                {/* Trunk */}
                <mesh position={[0, 1, 0]}>
            ...
            {decorations.houses.map((house, idx) => (
              <group 
                key={`house-${idx}`} 
                position={[house.position[0], house.position[1] + houseYOffset, house.position[2]]} 

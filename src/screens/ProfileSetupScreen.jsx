...
export function ProfileSetupScreen() {
  ...
  const filteredStarters = starters.filter((s) => {
    ...
    const matchesType =
      selectedType === "all" || s.types?.includes(selectedType);
  ...
  const toggleCompanion = (pokemon) => {
    const isSelected = selectedCompanions.some(
      (c) => c.entryId === pokemon.entryId
    );
    if (isSelected) {
      setSelectedCompanions(
        selectedCompanions.filter((c) => c.entryId !== pokemon.entryId)
      );
  ...
  const submit = async () => {
    ...
    try {
      ...
      setPlayer({
        ...newTrainer,
        coins: updatedUser.pokecoins ?? 500,
        userId: user.id,
      });
  ...
  const footer = (
    <ScreenFooter>
      <Button
        onClick={() =>
          wizard.isFirst ? goTo(SCREENS.dashboard) : wizard.back()
        }
      >
      ...
      <div className="profile-wizard-body">
        ...
        {wizard.step === 2 && (
          <div className="profile-step-companions">
            <p className="profile-step-hint">
              Selected ({selectedCompanions.length}/5)
            </p>
            ...
            <Pagination
              page={companionPager.page}
              totalPages={companionPager.totalPages}
              onPageChange={companionPager.setPage}
            />
            {previewCompanion && (
              <div
                className={`preview-canvas-container compact ${previewCompanion.types?.[0] || "normal"}`}
              >
                <CompanionPreview
        ...
        {wizard.step === 3 && (
          <div className="profile-step-style">
            ...
            />
            <Pagination
              page={stylePager.page}
              totalPages={stylePager.totalPages}
              onPageChange={stylePager.setPage}
            />
        ...
        {wizard.step === 4 && (
          <div className="profile-step-review">
            <div className="preview-details-card">
              ...
              <p>
                Companions:{" "}
                {selectedCompanions.map((c) => c.displayName).join(", ")}
              </p>
            </div>
            <div
              className={`preview-canvas-container compact ${previewCompanion?.types?.[0] || "normal"}`}

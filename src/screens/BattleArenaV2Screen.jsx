...
import { ArrowLeft, RotateCcw, ShieldCheck, Sparkles, Swords, TestTube2 } from 'lucide-react';
import { api } from '../api';
...
function parseCondition(condition) {
  ...
  return { currentHp: currentHp || 0, maxHp: maxHp || 100, status: statusPart || 'none' };
}
...
function getRequestTeamSlots(request, team) {
  ...
  return requestPokemon.map((pokemon, requestIndex) => {
    ...
    return { ...fixture, ...hp, active: Boolean(pokemon?.active), requestIndex, condition: pokemon?.condition };
  });
}
...
function formatLog(line) {
  ...
  switch (parts[1]) {
    ...
    case '-crit':
      return 'Critical hit.';
    case '-weather':
      return parts[3] === '[upkeep]' ? `Weather remains ${parts[2]}.` : `Weather changed to ${parts[2]}.`;
    case 'faint':
...
export function BattleArenaV2Screen() {
  ...
  const playerSlots = useMemo(() => getRequestTeamSlots(session?.requests?.p1, playerTeam), [session, playerTeam]);
  const rivalSlots = useMemo(() => getRequestTeamSlots(session?.requests?.p2, rivalTeam), [session, rivalTeam]);
  const player = useMemo(() => playerSlots.find((pokemon) => pokemon.active) || playerSlots[0], [playerSlots]);
  const rival = useMemo(() => rivalSlots.find((pokemon) => pokemon.active) || rivalSlots[0], [rivalSlots]);
  ...
  const handleDraftToggle = (poke) => {
    setSelectedTeam((prev) => {
      const isSelected = prev.some((selected) => selected.entryId === poke.entryId);
      if (isSelected) return prev.filter((selected) => selected.entryId !== poke.entryId);
  ...
  const fetchPokemonDetailsForEngine = async (entry, slotIndex) => {
    ...
    if (!res.ok) throw new Error(`Could not load ${entry.displayName || entry.name}`);
    const data = await res.json();
    const stats = {};
    data.stats.forEach((statRow) => { stats[statRow.stat.name] = statRow.base_stat; });

    const moves = [];
    for (const moveName of data.moves.slice(0, 4).map((moveRow) => moveRow.move.name)) {
      try {
  ...
  const startBattle = async () => {
    ...
    try {
      ...
      for (let index = 0; index < selectedTeam.length; index += 1) {
        resolvedTeam.push(await fetchPokemonDetailsForEngine(selectedTeam[index], index));
      }
      ...
      const result = await api.createBattleEngineSession({
        ...
        participants: [
          {
            side: 'p1',
            user: { id: user?.id || 'local-user', name: user?.displayName || user?.username || 'Player' },
            control: 'human',
            team: resolvedTeam,
          },
          { side: 'p2', user: { id: 'rival-v2', name: 'Rival V2', type: 'npc' }, control: 'ai', team: RIVAL_TEAM },
        ],
      });
      setPlayerTeam(result.participants.find((participant) => participant.side === 'p1')?.team.map((pokemon, index) => ({
        ...resolvedTeam[index],
        ...pokemon,
        sprite: resolvedTeam[index]?.sprite,
      })) || resolvedTeam);
      setRivalTeam(result.participants.find((participant) => participant.side === 'p2')?.team.map((pokemon, index) => ({
        ...RIVAL_TEAM[index],
        ...pokemon,
      })) || RIVAL_TEAM);
      setSession(result);
  ...
  const submitChoice = async (choice) => {
    ...
    setError('');
    try {
      const result = await api.submitBattleEngineChoice(session.battleId, { side: 'p1', choice });
      setSession(result);
      appendLogs(result.logs);
      await loadMoveCatalog(result.requests?.p1?.active?.[0]?.moves || []);
    } catch (err) {
      ...
      <header className="minigames-header">
        ...
        <div className="minigames-header-stats">
          <div className="minigames-stat-badge accent">
            <ShieldCheck size={15} />
            <span>{session ? session.battleId.slice(0, 8) : `${selectedTeam.length}/3 Drafted`}</span>
          </div>
          <button type="button" className="btn-back" onClick={() => goTo(SCREENS.minigameHub)}>
        ...
        {stage === 'draft' ? (
          <section className="minigame-container battle-v2-draft">
            <div className="minigame-content" style={{ textAlign: 'left' }}>
              <h2 className="minigame-inner-title">Draft Your V2 Engine Squad</h2>
              <p className="minigame-inner-subtitle" style={{ marginBottom: '20px' }}>
                Choose 3 Pokemon. V2 resolves their stats and moves into explicit battle-ready engine payloads.
              </p>

              <div className="battle-v2-toolbar" style={{ marginBottom: '24px' }}>
                ...
                <div className="battle-arena-config">
                  ...
                  <div className="battle-v2-selected-strip">
                    {selectedTeam.length ? selectedTeam.map((pokemon) => (
              ...
              <div className="minigames-bento-grid battle-v2-draft-grid">
                {draftPool.map((pokemon) => {
                  const isSelected = selectedTeam.some((selected) => selected.entryId === pokemon.entryId);
                  return (
                    <button
                      key={pokemon.entryId}
                      type="button"
                    ...
                    >
                      <span className="battle-v2-draft-icon">{starterIcon(pokemon.types)}</span>
                      <span className="species-label">{pokemon.displayName}</span>
                      <span className="battle-v2-draft-types">{pokemon.types.join(' / ')}</span>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="battle-v2-draft-footer">
                <button
                  ...
                  onClick={startBattle}
                >
                  <Swords size={15} /> {loading ? 'Resolving Team...' : 'Start V2 Battle'}
                </button>
              </div>
            </div>
          </section>
          ...
          <>
            <section className="battle-v2-toolbar">
              <div className="battle-v2-weather">
                {WEATHER_OPTIONS.map((option) => (
                  <button key={option.id} type="button" className={`config-btn ${weather === option.id ? 'active' : ''}`} disabled>
                    {option.label}
                  </button>
                ))}
              </div>
              <button type="button" className="btn-back battle-v2-start" disabled={loading} onClick={restartDraft}>
            ...
            <section className="battle-v2-stage">
              <div className="battle-v2-combatant rival">
                ...
                <img className="battle-v2-sprite" src={rival?.sprite} alt={rival?.displayName} />
                <div className="battle-v2-meta"><span>{rival?.ability}</span><span>{rival?.item || 'No Item'}</span></div>
              </div>
              ...
              <div className="battle-v2-combatant player">
                ...
                <img className="battle-v2-sprite flip" src={player?.sprite} alt={player?.displayName} />
                <div className="battle-v2-meta"><span>{player?.ability}</span><span>{player?.item || 'No Item'}</span></div>
              </div>
            </section>
            ...
            <section className="battle-v2-actions">
                ...
                {requestMoves.map((move, index) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!session || session.ended || loading || move.disabled}
              ...
              <div className="battle-v2-side-actions">
                <button type="button" className="btn-menu-action" disabled={!session || loading || items.potions <= 0} onClick={usePotion}>
                  <TestTube2 size={14} /> Potion ({items.potions})
                </button>
                <button type="button" className="btn-menu-action" disabled={!session || loading || items.fullRestores <= 0} onClick={useFullRestore}>
                  <Sparkles size={14} /> Full Restore ({items.fullRestores})
                </button>
                {playerSlots.map((pokemon) => {
                  const isActive = Boolean(pokemon.active);
                  const isFainted = pokemon.status === 'fnt' || pokemon.condition?.includes('fnt');
                  return (
                    <button
                      key={pokemon.id}
                      type="button"
            ...
            <section className="battle-logs-card battle-v2-logs">
              <span className="battle-logs-title">{session?.ended ? `Winner: ${session.winner}` : 'Battle Logs'}</span>
              <div className="battle-logs-stream">
                {(logs.length ? logs : ['Start a V2 battle.']).map((line, index) => (
                  <div key={`${line}-${index}`} className="battle-log-line">{line}</div>
                ))}
              </div>
            </section>
          </>

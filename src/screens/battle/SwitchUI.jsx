...
import { findRequestPokemonForTeamIndex, getRequestSlotForTeamIndex } from './BattleUtils';
...
export function SwitchUI({
  ...
  const hasAvailableSwitch = playerTeam.some((p, i) => {
    ...
    const isAlreadyChosen = turnChoicesRef.current.includes(`switch ${requestSlotIndex + 1}`);
  ...
  return (
    <div className="bl-moves-panel" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      <div className="bl-moves-header" style={{ marginBottom: '12px' }}>
        ...
        {activeRequest?.forceSwitch && <span className="bl-timer-badge">Required</span>}
      </div>
      <div className="bl-swap-list" style={{ overflowY: 'auto' }}>
        ...
        {!hasAvailableSwitch && activeRequest?.forceSwitch && (
          ...
          >
            ...
            <span className="bl-swap-info">
              <span className="bl-swap-name" style={{ color: 'var(--px-red)' }}>Skip Switch (No Pokémon)</span>
              <span className="bl-swap-bar-row">

...
export function findTeamIndexForRequestPokemon(
  requestPokemon,
  team,
  fallbackIndex = 0
) {
  ...
  const reqSpecies = normalizePokemonKey(
    speciesFromDetails(requestPokemon.details)
  );
...
export function getRequestSlotForTeamIndex(
  activeRequest,
  team,
  targetTeamIndex
) {
  ...
  const targetName = normalizePokemonKey(
    targetPokemon.displayName || targetPokemon.name
  );
  ...
  for (let i = 0; i < activeRequest.side.pokemon.length; i++) {
    ...
    if (
      reqName === targetName ||
      reqName === targetSpecies ||
      reqSpecies === targetName ||
      reqSpecies === targetSpecies
    ) {
      return i;
    }
  }
...
export function findRequestPokemonForTeamIndex(
  activeRequest,
  team,
  targetTeamIndex
) {
  if (!activeRequest?.side?.pokemon) return null;
  const slotIdx = getRequestSlotForTeamIndex(
    activeRequest,
    team,
    targetTeamIndex
  );
...
export function findTeamIndexForBattleIdent(
  ident,
  team,
  request,
  fallbackIndex = 0
  ...
  const directIndex = team.findIndex((pokemon) =>
    [pokemon.displayName, pokemon.name, pokemon.species, pokemon.id]
      .map(normalizePokemonKey)
      .includes(identKey)
  );
  if (directIndex >= 0) return directIndex;

  const requestPokemon = (request?.side?.pokemon || []).find(
    (pokemon) => normalizePokemonKey(cleanName(pokemon.ident)) === identKey
  );
  return requestPokemon
    ? findTeamIndexForRequestPokemon(requestPokemon, team, fallbackIndex)
    : fallbackIndex;
}

export function getActiveTeamIndexFromRequest(
  request,
  team,
  fallbackIndex = 0
) {
  ...
  return findTeamIndexForRequestPokemon(
    requestPokemon[requestIndex],
    team,
    requestIndex
  );
}
...
export function syncTeamFromRequest(team, request) {
  ...
  request?.side?.pokemon?.forEach((requestPokemon, requestIndex) => {
    const teamIndex = findTeamIndexForRequestPokemon(
      requestPokemon,
      nextTeam,
      requestIndex
    );

...
import { api } from "../api";
import { ArrowLeft, Swords, RotateCcw, ChevronDown, Check } from "lucide-react";
...
const TYPE_CHART = {
  ...
  grass: {
    ...
    halfDamageFrom: ["water", "grass", "electric", "ground"],
    noDamageFrom: [],
  },
  electric: {
  ...
  flying: {
    ...
    halfDamageFrom: ["fighting", "grass", "bug"],
    noDamageFrom: ["ground"],
  },
  psychic: {
...
const WEATHER_OPTIONS = [
  {
    ...
    icon: "🎲",
    color: "text-zinc-400",
    desc: "Weather chosen randomly each battle.",
  },
  {
    id: "clear",
    name: "Clear Sky",
    icon: "☀️",
    color: "text-zinc-400",
    desc: "Standard battle conditions.",
  },
  {
    id: "sun",
    name: "Harsh Sunlight",
    ...
    desc: "Fire moves ×1.5. Water moves ×0.5.",
  },
  {
    id: "rain",
    name: "Heavy Rain",
    icon: "💧",
    color: "text-blue-400",
    desc: "Water moves ×1.5. Fire moves ×0.5.",
  },
  {
    id: "sandstorm",
    name: "Sandstorm",
    ...
    desc: "Rock-type Sp. Def boosted by 1.5×.",
  },
  {
    id: "hail",
    name: "Snow / Hail",
...
const DIFFICULTIES = [
  {
    ...
    icon: "🌿",
    rating: "Easy",
    desc: "Common Pokémon, low level. Great for practice.",
    coins: 30,
  },
  {
    id: "gym",
    ...
    icon: "🏅",
    rating: "Medium",
    desc: "Trainer-level threats with solid movesets.",
    coins: 75,
  },
  {
    id: "boss",
    name: "Legendary Raid Boss",
    icon: "💀",
    rating: "Hard",
    desc: "Elite legendaries at max power.",
    coins: 150,
  },
  {
    id: "trainer3v3",
    ...
    icon: "🎮",
    rating: "3v3",
    desc: "Face a random NPC trainer team.",
    coins: 200,
  },
];
    ...
    <div className="battle-dropdown-root" ref={ref} id={id}>
      ...
      >
        ...
        <span className="battle-dropdown-trigger-text">
          ...
          {selected.rating && (
            <span className="battle-dropdown-trigger-badge">
              {selected.rating}
            </span>
          )}
        </span>
        <ChevronDown
          size={14}
          className={`battle-dropdown-chevron ${open ? "rotated" : ""}`}
        />
      </button>
              ...
              >
                ...
                <span className="battle-dropdown-item-body">
                  <span className="battle-dropdown-item-name">
                    {opt.name}
                    {opt.rating && (
                      <span className="battle-dropdown-item-badge">
                        {opt.rating}
                      </span>
                    )}
                  </span>
                  {opt.desc && (
                    <span className="battle-dropdown-item-desc">
                      {opt.desc}
                    </span>
                  )}
                </span>
                {isActive && (
                  <Check size={13} className="battle-dropdown-item-check" />
                )}
              </button>
            );
          })}
...
function formatShowdownLog(line) {
  ...
  switch (type) {
    ...
    case "-heal": {
      ...
      const reason = parts[4] || "";
      if (reason.includes("[silent]")) return null;
      if (reason.includes("[from] drain"))
        return `🧛 <strong>${target}</strong> drained some energy!`;

      const item = reason.includes("[from] item:")
        ? reason.replace("[from] item: ", "")
        : "";
      if (item) {
...
export function BattleArenaScreen() {
  ...
  const currentFormat =
    BATTLE_FORMATS.find((f) => f.id === battleFormat) || BATTLE_FORMATS[0];
  ...
  const _setBattleId = (v) => {
    battleIdRef.current = v;
    setBattleId(v);
  };
  const _setPlayerSlots = (v) => {
    const next = typeof v === "function" ? v(playerSlotsRef.current) : v;
    playerSlotsRef.current = next;
    setPlayerSlots(next);
  };
  const _setEnemySlots = (v) => {
    const next = typeof v === "function" ? v(enemySlotsRef.current) : v;
    enemySlotsRef.current = next;
    setEnemySlots(next);
  };
  const _setChoosingSlotIdx = (v) => {
    const next = typeof v === "function" ? v(choosingSlotIdxRef.current) : v;
    choosingSlotIdxRef.current = next;
    setChoosingSlotIdx(next);
  };
  const _setTurnChoices = (v) => {
    const next = typeof v === "function" ? v(turnChoicesRef.current) : v;
    turnChoicesRef.current = next;
    setTurnChoices(next);
  };
  const _setActiveRequest = (v) => {
    activeRequestRef.current = v;
    setActiveRequest(v);
  };
  const _setPlayerTeam = (v) => {
    const next = typeof v === "function" ? v(playerTeamRef.current) : v;
    playerTeamRef.current = next;
    setPlayerTeam(next);
  };
          ...
          if (prev <= 1) {
            ...
            const firstAvailableIdx = playerTeam.findIndex((p, i) => {
              ...
              const isAlreadyChosen =
                turnChoicesRef.current.includes(choiceStr);
            ...
            if (firstAvailableIdx !== -1) {
              ...
              const forceSwitchArray = Array.isArray(activeRequest.forceSwitch)
  ...
  const getRequestMoves = useCallback(() => {
    if (
      ...
      !activeRequest.active[choosingSlotIdx]
    )
      return [];
  ...
  useEffect(() => {
    ...
    const currentActivePoke =
      currentActiveIdx !== null ? playerTeam[currentActiveIdx] : null;
    if (stage === "battle" && currentActivePoke && enemy && activeRequest) {
      ...
      setCalcInputs((prev) => ({
        ...
        stab: currentActivePoke.types.includes(defaultMoveDetail.type)
          ? 1.5
          : 1.0,
      ...
      for (const line of result.logs) {
        if (line.startsWith("|switch|") || line.startsWith("|drag|")) {
          ...
          if (target.startsWith("p1")) {
            ...
            const targetTeamIdx = findTeamIndexForBattleIdent(
              target,
              resolvedTeam,
              result.request,
              slotIdx
  ...
  const handleDifficultySelect = (nextDifficulty) => {
    ...
    if (
      nextDifficulty === "boss" &&
      (battleFormat === "doubles" || battleFormat === "triples")
    ) {
      setBattleFormat("singles3v3");
      setSelectedTeam([]);
    }
    if (
      nextDifficulty === "trainer3v3" &&
  ...
  const isValidTarget = (sourceIdx, targetType, targetIsFoe, targetIdx) => {
    if (battleFormat === "singles" || battleFormat === "doubles") {
      if (targetType === "adjacentAlly")
        return !targetIsFoe && targetIdx !== sourceIdx;
      ...
      if (targetType === "normal" || targetType === "any")
        return targetIdx !== sourceIdx || targetIsFoe;

      // Multi-target logic checking
      if (targetType === "allAdjacent")
        return targetIsFoe || targetIdx !== sourceIdx;
      if (targetType === "allAdjacentFoes" || targetType === "foeSide")
        return targetIsFoe;
      if (targetType === "allySide")
        return !targetIsFoe && targetIdx !== sourceIdx;
    ...
    if (targetType === "adjacentAllyOrSelf")
      return (!targetIsFoe && isAdjacent) || targetIdx === sourceIdx;
    ...
    if (targetType === "allAdjacent")
      return targetIdx !== sourceIdx && isAdjacent;
  ...
  const playTurnEvents = async (logs, nextRequest, nextWinner) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (type === "move") {
        ...
        if (actor.startsWith("p1")) {
          ...
          if (moveId.includes("electric") || moveId.includes("thunder"))
            playAudioTone("spark");
          else if (moveId.includes("slash") || moveId.includes("wing"))
            playAudioTone("slash");
        ...
        if (target.startsWith("p1")) {
          setPlayerAnim("hit");
          const targetTeamIndex = findTeamIndexForBattleIdent(
            target,
            localTeam,
            nextRequest,
            slotIdx
        ...
        if (target.startsWith("p1")) {
          ...
          if (localTeam[targetTeamIndex])
            localTeam[targetTeamIndex].status = status;
          _setPlayerTeam([...localTeam]);
        } else if (target.startsWith("p2")) {
          if (localEnemySlots[slotIdx])
            localEnemySlots[slotIdx].status = status;
          _setEnemySlots([...localEnemySlots]);
        }
        ...
        if (target.startsWith("p1")) {
          ...
          if (localTeam[targetTeamIndex])
            localTeam[targetTeamIndex].status = "none";
          _setPlayerTeam([...localTeam]);
        } else if (target.startsWith("p2")) {
          if (localEnemySlots[slotIdx])
            localEnemySlots[slotIdx].status = "none";
          _setEnemySlots([...localEnemySlots]);
        }
      } else if (type === "faint") {
        const target = parts[2];
        ...
        if (target.startsWith("p1")) {
          ...
          if (localTeam[targetTeamIndex])
            localTeam[targetTeamIndex].currentHp = 0;
          _setPlayerTeam([...localTeam]);
        ...
        const sourceSlotIdx =
          sourceSlotChar === "a" ? 0 : sourceSlotChar === "b" ? 1 : 2;
        const targetSlotChar = target.charAt(2);
        const targetSlotIdx =
          targetSlotChar === "a" ? 0 : targetSlotChar === "b" ? 1 : 2;
        ...
        if (source.startsWith("p1") && target.startsWith("p2")) {
          ...
          if (localEnemySlots[sourceSlotIdx]) {
            ...
            const playerTarget =
              playerTargetIdx !== null ? localTeam[playerTargetIdx] : null;
    ...
    if (nextWinner) {
      ...
      if (nextWinner === "player") {
        const payout =
          DIFFICULTIES.find((entry) => entry.id === difficulty)?.coins || 150;
        addCoins(payout);
      ...
      if (nextRequest?.forceSwitch) {
        ...
        let firstSlotIdx = 0;
        while (
          firstSlotIdx < currentFormat.activeCount &&
          (!nextRequest?.active?.[firstSlotIdx] ||
            localPlayerSlots[firstSlotIdx] === null ||
            localTeam[localPlayerSlots[firstSlotIdx]]?.currentHp === 0)
        ) {
        ...
        _setChoosingSlotIdx(
          firstSlotIdx < currentFormat.activeCount ? firstSlotIdx : 0
        );
      }
    }
  ...
  const proceedToNextSlot = async (choice, forceSwitchArray = null) => {
    ...
    if (isForcedSwitch) {
      ...
      while (
        nextSlotIdx < forceSwitchArray.length &&
        !forceSwitchArray[nextSlotIdx]
      ) {
      ...
      if (
        nextSlotIdx < forceSwitchArray.length &&
        getAvailableSwitchCount(nextChoices) > 0
      ) {
        ...
        setLoading(true);
        try {
          const result = await api.submitBattleChoice({
            battleId: battleIdRef.current,
            choice: combinedChoice,
          });
      ...
      let nextSlotIdx = choosingSlotIdxRef.current + 1;
      while (
        nextSlotIdx < activeCount &&
        (!currentActiveRequest?.active?.[nextSlotIdx] ||
          currentPlayerSlots[nextSlotIdx] === null ||
          currentPlayerTeam[currentPlayerSlots[nextSlotIdx]]?.currentHp === 0)
      ) {
  ...
  const handlePlayerAttack = async (moveSlotIndex) => {
    ...
    const isMultiTarget =
      currentFormat.activeCount >= 2 && multiTargetTypes.includes(targetType);
  ...
  const handleSelectTarget = (targetIsFoe, targetIdx) => {
    ...
    const targetLoc = targetIsFoe ? targetIdx + 1 : -(targetIdx + 1);
  ...
  const handleSwapActive = async (idx) => {
    if (!playerTurn || isActing || winner !== null || playerSlots.includes(idx))
      return;
    setShowSwitch(false);

    const forceSwitchArray = activeRequest?.forceSwitch
      ? Array.isArray(activeRequest.forceSwitch)
        ? activeRequest.forceSwitch
        : [!!activeRequest.forceSwitch]
      : null;
  ...
  const handleUseItem = async (type) => {
    ...
    setItems((prev) => ({
      ...
      fullRestores:
        type === "fullRestore" ? prev.fullRestores - 1 : prev.fullRestores,
    }));
    setBagOpen(false);
  ...
  const handleConfirmUseItem = (type, targetIdx) => {
    const choice =
      type === "potion" ? `potion ${targetIdx}` : `fullrestore ${targetIdx}`;
  ...
  const getMappedMoves = () => {
    const currentActivePoke =
      playerSlots[choosingSlotIdx] !== null
        ? playerTeam[playerSlots[choosingSlotIdx]]
        : null;
    return getRequestMoves().map((reqMove) => {
      const allMoves =
        currentActivePoke?.transformData?.moves ||
        currentActivePoke?.moves ||
        [];
  ...
  const calcOutput = () => {
    const base =
      (((2 * calcInputs.level) / 5 + 2) *
        calcInputs.power *
        (calcInputs.atk / calcInputs.def)) /
        50 +
      2;
    const modifier =
      calcInputs.stab *
  ...
  return (
    <div className="minigames-screen" id="battle-arena-screen">
      <header className="minigames-header">
        ...
        <div
          className="minigames-header-stats"
          style={{ display: "flex", gap: "12px" }}
        >
          {stage !== "draft" && (
            <button
              className="btn-back flee"
              id="flee-arena-btn"
      ...
      {loading ? (
        ...
        >
          <p className="minigames-eyebrow animate-pulse">
            Summoning battle matrices & loading assets...
          </p>
        </div>
      ) : stage === "draft" ? (
          ...
          <div className="minigame-content" style={{ textAlign: "left" }}>
            ...
            >
              <div>
                <h2 className="minigame-inner-title">
                  Draft Your Squad (Choose {currentFormat.teamSize})
                </h2>
                <p
                  className="minigame-inner-subtitle"
                  style={{ marginBottom: 0 }}
                >
                  Select {currentFormat.teamSize} Pokémon for a{" "}
                  <strong>{currentFormat.name}</strong>.
                </p>
              </div>
            </div>
            ...
            >
              <button
                ...
                disabled={
                  selectedTeam.length < currentFormat.activeCount || loading
                }
                onClick={() => startBattle()}
                style={{
                  background:
                    selectedTeam.length >= currentFormat.activeCount
                      ? "var(--px-sky)"
        ...
        <div className="battle-layout-v2">
          ...
          <div className="bl-stage-col">
            <div className="battle-stage" id="battle-stage-arena">
              ...
              {/* Enemy Side */}
              <div
                className="stage-pokemon-row"
                style={{ justifyContent: "flex-end", gap: "20px" }}
              >
                {Array.from({ length: currentFormat.activeCount }).map(
                  (_, i) => {
                    ...
                    if (!activeEnemy)
                      return (
                        <div
                          key={slotIdx}
                          className="stage-pokemon-container empty"
                          style={{
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          isTargetable && handleSelectTarget(true, slotIdx)
                        }
                        style={{
                          width: `${90 / currentFormat.activeCount}%`,
                      ...
                      >
                        ...
                        >
                          <div
                            className="hp-panel-header"
                            style={{ justifyContent: "flex-start" }}
                          >
                            <span
                              className="hp-pokemon-level"
                              style={{ marginRight: "8px" }}
                            >
                              Lvl {activeEnemy.level || 50}
                            </span>
                            <span className="hp-pokemon-name">
                              {activeEnemy.gender && (
                                <span
                                  style={{ opacity: 0.7, marginRight: "4px" }}
                                >
                                  {activeEnemy.gender}
                                </span>
                              )}{" "}
                              {activeEnemy.transformData?.displayName ||
                                activeEnemy.displayName}
                            </span>
                          </div>
                          <div className="hp-bar-outer">
                            <div
                              ...
                              style={{
                                width: `${(activeEnemy.currentHp / activeEnemy.maxHp) * 100}%`,
                              }}
                            />
                          </div>
                          <div className="hp-numeric">
                            {activeEnemy.status !== "none" && (
                              <span
                        ...
                        >
                          <img
                            src={
                              activeEnemy.transformData?.sprite ||
                              activeEnemy.sprite
                            }
                            alt={
                              activeEnemy.transformData?.displayName ||
                  ...
                  (_, slotIdx) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          isTargetable && handleSelectTarget(false, slotIdx)
                        }
                        style={{
                          width: `${90 / currentFormat.activeCount}%`,
                          cursor: isTargetable ? "pointer" : "default",
                      ...
                      >
                        <div
                          ...
                          style={{
                            border: isChoosing
                              ? "2px solid var(--px-sky)"
                              : isTargetable
                                ? "2px solid var(--px-accent)"
                                : "1px solid var(--px-border)",
                            boxShadow: isChoosing
                        ...
                        >
                          <div className="hp-panel-header">
                            <span className="hp-pokemon-name">
                              {activePoke.transformData?.displayName ||
                          ...
                          <div className="hp-bar-outer">
                            <div
                              ...
                              style={{
                                width: `${(activePoke.currentHp / activePoke.maxHp) * 100}%`,
                              }}
                            />
                          </div>
                          <div className="hp-numeric">
                            {activePoke.currentHp} / {activePoke.maxHp} HP
                            {activePoke.status !== "none" && (
                              <span
                                className="status-tag"
                                style={{
                                  marginLeft: "6px",
                        ...
                        <div
                          ...
                          style={
                            isChoosing
                              ? {
                                  filter: "drop-shadow(0 0 10px var(--px-sky))",
                                  transform: "scaleX(-1)",
                                }
                              : { transform: "scaleX(-1)" }
                          }
                        >
                          <img
                            src={
                              activePoke.transformData?.sprite ||
                              activePoke.sprite
                            }
                            alt={
                              activePoke.transformData?.displayName ||
                ...
                <div className="bl-target-select-grid">
                    ...
                    (_, slotIdx) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            !targetSelection.isMultiTarget &&
                            handleSelectTarget(true, slotIdx)
                          }
                          style={
                            targetSelection.isMultiTarget
                              ? {
                        ...
                        >
                          ...
                          <div className="bl-target-card-info">
                            <span className="bl-target-card-name">
                              {foe.transformData?.displayName ||
                                foe.displayName}{" "}
                              <small
                            ...
                            <span className="bl-target-card-hp">
                  ...
                  {Array.from({ length: currentFormat.activeCount }).map(
                    (_, slotIdx) => {
                      ...
                      if (!ally) return null;
                      const canTarget = targetSelection.itemType
                        ? true
                        : isValidTarget(
                            choosingSlotIdx,
                            targetSelection.targetType,
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            !targetSelection.isMultiTarget &&
                            handleSelectTarget(false, slotIdx)
                          }
                          style={
                            targetSelection.isMultiTarget && !isSelf
                              ? {
                                  cursor: "default",
                        ...
                        >
                          <div className="bl-target-card-badge ally">
                            {isSelf ? "SELF" : "ALLY"}
                          </div>
                          <img
                          ...
                          <div className="bl-target-card-info">
                            <span className="bl-target-card-name">
                              {ally.transformData?.displayName ||
                                ally.displayName}{" "}
                              <small
                                style={{ opacity: 0.8, fontSize: "0.8em" }}
                              >
                ...
                {targetSelection.isMultiTarget && (
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        proceedToNextSlot(
                          `move ${targetSelection.moveSlotIndex + 1}`
                        );
                        setTargetSelection(null);
                      }}
                    >

...
function createThrowArc(player, camera, throwPower, currentBiome, caveZone) {
  ...
  for (let t = 0; t <= MAX_SIMULATION_TIME; t += SIMULATION_STEP) {
    ...
    const groundY =
      getTerrainSurfaceY(point.x, point.z, currentBiome, caveZone) +
      PROJECTILE_RADIUS;
...
export default function AimIndicator({
  ...
  useFrame(({ camera }) => {
    ...
    setArc(createThrowArc(player, camera, throwPower, currentBiome, caveZone));
  });

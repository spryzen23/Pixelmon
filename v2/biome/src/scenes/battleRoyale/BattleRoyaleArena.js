...
import IceMountainLandmarks from '../../biomes/icy/IceMountainLandmarks';
...
const PLAYER_MODEL_URL = '/assets/shared/player.glb';
...
function BattleRoyaleCatchLayer({
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.repeat || (event.code !== 'Space' && event.code !== 'KeyF') || !playerRef.current) {
        return;
      }
        ...
        return (
          <Suspense key={creature.id} fallback={null}>
            <WildCreature
              ...
              modelUrl={asset.url || '/assets/shared/wild_creature.glb'}
              playerRef={playerRef}

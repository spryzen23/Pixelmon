...
export function DailyGridScreen() {
  ...
  const saveSessionState = (updatedGrid, updatedGuesses, isOver, isWon, isSurrendered) => {
    const utcDateStr = new Date().toISOString().split('T')[0];
  ...
  const getCellCandidateNames = (cellIdx) => {
    ...
    const matches = speciesList.filter(p => matchesCategory(p, row) && matchesCategory(p, col));
            ...
            {gameOver && (
              <div className="grid-status-alert" style={{ background: won ? 'rgba(28,212,93,0.08)' : 'rgba(248,79,79,0.08)', borderColor: won ? 'rgba(28,212,93,0.2)' : 'rgba(248,79,79,0.2)' }}>
                <div style={{ textAlign: 'left' }}>
                  ...
                  <p>{won ? 'Congratulations! You solved all 9 coordinates of today\'s PokéGrid!' : 'Try again tomorrow to solve the coordinates!'}</p>
                </div>
              </div>
            )}

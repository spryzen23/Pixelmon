...
const MULTIPLIERS = {
  normal: { fire: 1, water: 1, grass: 1, electric: 1, normal: 1, rock: 0.5, steel: 0.5, ghost: 0 },
  fire: { fire: 0.5, water: 0.5, grass: 2, electric: 1, ice: 2, bug: 2, rock: 0.5, dragon: 0.5, steel: 2 },
  water: { fire: 2, water: 0.5, grass: 0.5, electric: 1, ground: 2, rock: 2, dragon: 0.5 },
  grass: { fire: 0.5, water: 2, grass: 0.5, poison: 0.5, ground: 2, flying: 0.5, bug: 0.5, rock: 2, dragon: 0.5, steel: 0.5 },
  electric: { water: 2, grass: 0.5, electric: 0.5, ground: 0, flying: 2, dragon: 0.5 },
  ice: { fire: 0.5, water: 0.5, grass: 2, ice: 0.5, ground: 2, flying: 2, dragon: 2, steel: 0.5 },
  fighting: { normal: 2, ice: 2, rock: 2, dark: 2, steel: 2, flying: 0.5, poison: 0.5, psychic: 0.5, bug: 0.5, fairy: 0.5, ghost: 0 },
...
export function TriviaTrainingScreen() {
  ...
  // Global Timer Hook
  useEffect(() => {
    if ((mode === 'speedrun' || mode === 'silhouette') && timeLeft > 0 && !quizOver) {
    ...
    <div className="minigames-screen">
      ...
      {loading ? (
        <div className="minigame-inner-header" style={{ border: 'none', justifyContent: 'center', minHeight: '300px' }}>
          <p className="minigames-eyebrow animate-pulse">Preloading stats sheets...</p>
        </div>
      ) : mode === 'menu' ? (
          ...
          <div className="minigames-bento-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
            ...
            <div className="bento-card featured" onClick={startSpeedRun} style={{ minHeight: '160px' }}>
              ...
              <div>
                ...
                <p className="bento-desc">Answer YES/NO typing questions against a strict 60-second lightning timer!</p>
              </div>
            </div>
            ...
            <div className="bento-card featured" onClick={startTypeExpert} style={{ minHeight: '160px' }}>
              ...
              <div>
                ...
                <p className="bento-desc">Test your knowledge of damage multipliers between elemental typings (10 Questions).</p>
              </div>
            </div>
            ...
            <div className="bento-card" onClick={startSilhouette} style={{ minHeight: '160px' }}>
              ...
              <div>
                ...
                <p className="bento-desc">Type names of Pokémon matching their pitch-black silhouette outline within 60 seconds.</p>
              </div>
            </div>
            ...
            <div className="bento-card" onClick={startTypeFocus} style={{ minHeight: '160px' }}>
              ...
              <div>
                ...
                <p className="bento-desc">Identify dual-type Pokémon from multiple-choice lists (10 Questions).</p>
              </div>
            </div>
        ...
        <div className="minigame-container" style={{ maxWidth: '480px', margin: '0 auto' }}>
          <div className="minigame-content" style={{ textAlign: 'center', padding: '40px 24px' }}>
        ...
        <div className="minigame-container" style={{ maxWidth: '540px', margin: '0 auto' }}>
          <div className="minigame-inner-header">
            <div>
              <span className="minigames-eyebrow">Lab Active</span>
              <h2 className="minigame-inner-title" style={{ textTransform: 'capitalize' }}>
                {mode.replace('expert', ' Expert').replace('run', ' Run')}
              </h2>
            </div>
            {mode === 'typeexpert' || mode === 'typefocus' ? (
              <span style={{ fontSize: '12px', fontWeight: 700 }}>Q: {questionIdx + 1} / 10</span>
            ) : null}
          </div>
          ...
          <div className="minigame-content" style={{ padding: '30px 24px' }}>
            ...
            {mode === 'silhouette' && activeQuestion && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
                ...
                {showSilhouetteName ? (
                  <p style={{ fontSize: '18px', fontWeight: 800, color: 'var(--px-success)', textTransform: 'capitalize', margin: 0 }}>
                  ...
                  <form onSubmit={handleSilhouetteSubmit} style={{ display: 'flex', gap: '8px', width: '100%', maxWidth: '340px' }}>
                    <input 
                      ...
                      autoFocus
                    />
                    <button type="submit" className="btn-back" style={{ background: 'var(--px-sky)', borderColor: 'var(--px-sky)' }}>Submit</button>
                  </form>
                )}
              </div>
            )}

...
with st.sidebar:
    ...
    real_time_sync = st.checkbox("Enable Real-Time Sync (2s)", value=False)
    if real_time_sync:
        time.sleep(2)
        st.rerun()
...
st.markdown("## 🚀 First Screen Load Latency")
first_load_avg = average_duration(first_loads)
st.metric(
    "Average First Load",
    format_ms(first_load_avg),
    help="The time it takes to fully hydrate and render a biome for the very first time."
)
...
metric_2.metric("Average Wait (All)", format_ms(average_duration(df)))
metric_3.metric("Average Cached Reload", format_ms(average_duration(cached_loads)))
